...
object StatusPlay : ModInitializer {
	...
	override fun onInitialize() {
		...
		try {
			gson.fromJson(file.readText(), JsonArray().javaClass).forEach {
				...
				val text = Text.Serializer.fromJson(obj["text"].asString)
			...
			dispatcher.register(
				CommandManager.literal("status").then(
					CommandManager.argument("status", StringArgumentType.word()).suggests(
						...
						server.scoreboard.addPlayerToTeam(source.name, server.scoreboard.getTeam(team.name))
					...
					val server = source.server
					server.scoreboard.removePlayerFromTeam(source.name, server.scoreboard.getPlayerTeam(source.name))
				...
				CommandManager.literal("ms").requires { it.player?.hasPermissionLevel(2) != false }.then(
					CommandManager.literal("add").then(
						CommandManager.argument(
							...
							addStatus(id, text, it.source.server)
							source.sendFeedback({ Text.literal("Added status ").append(text) }, false)
							1
						}))).then(CommandManager.literal("custom").then(CommandManager.argument("text", TextArgumentType.text()).executes {
							val source = it.source
							val id = it.getArgument("id", String::class.java)
							val text = it.getArgument("text", Text::class.java)
							addStatus(id, text.copy(), it.source.server)
							source.sendFeedback({ Text.literal("Added status ").append(text) }, false)
							1
						}))
					)
					...
					CommandManager.literal("remove").then(
						CommandManager.argument(
							...
							removeStatus(id, it.source.server)
					...
					CommandManager.literal("set").then(
						CommandManager.argument(
							...
							it.source.server.scoreboard.addPlayerToTeam(player.gameProfile.name, it.source.server.scoreboard.getTeam(team.name))
							...
							val player = EntityArgumentType.getPlayer(it, "player")
							val server = source.server
							server.scoreboard.removePlayerFromTeam(player.gameProfile.name, server.scoreboard.getPlayerTeam(player.gameProfile.name))
	...
	private fun addStatus(text: String, name: MutableText, server: MinecraftServer) {
		statusList.removeIf { it.name == text }
		statusList.add(Status(text.lowercase().replace(" ","_"), name))
		writeStatus()
		reloadStatusTeams(server.scoreboard)
	}

	private fun removeStatus(name: String, server: MinecraftServer) {
	...
	private fun writeStatus() {
		...
		file.writeText(gson.toJson(statusList.map { status: Status ->
			...
			obj.addProperty("text", Text.Serializer.toJson(status.text))
			obj
		}))
	}
	...
	fun reloadStatusTeams(scoreboard: ServerScoreboard) {
